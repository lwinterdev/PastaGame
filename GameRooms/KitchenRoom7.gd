...
func _ready():
	...
	if GlobalRoomHandler.kitchen_room7_new == false:
		GlobalRoomHandler.load_room(player,room_name)
	
	GlobalRoomHandler.kitchen_room7_new = false
	GlobalRoomHandler.kitchen_room7.player_spawn_position = player.position
